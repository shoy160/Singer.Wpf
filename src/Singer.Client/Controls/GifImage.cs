    ...
    public class GifImage : UserControl
    {
        ...
        private void WebResponseCallback(IAsyncResult asyncResult)
        {
            ...
            try
            {
                ...
                webReadState.readStream.BeginRead(webReadState.buffer, 0, webReadState.buffer.Length,
                    new AsyncCallback(WebReadCallback), webReadState);
            }
        ...
        private void WebReadCallback(IAsyncResult asyncResult)
        {
            ...
            if (count > 0)
            {
                ...
                {
                    this.Dispatcher.Invoke(DispatcherPriority.Render, new WebRequestErrorDelegate(WebRequestError),
                        exp);
                }
            }
            else
            {
                this.Dispatcher.Invoke(DispatcherPriority.Render, new WebRequestFinishedDelegate(WebRequestFinished),
